...
class TestClassifiers(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.train_visits = pd.DataFrame({
            ...
            'sex': pd.Series(np.random.choice(['M', 'F'], n_samples), dtype='string'),
            'arrival_method': pd.Series(np.random.choice(['ambulance', 'walk-in', 'referral'], n_samples), dtype='string'),
            'is_admitted': np.random.choice([0, 1], n_samples, p=[0.7, 0.3]),
            'snapshot_time': pd.date_range(start='2023-01-01', periods=n_samples, freq='h'),
        ...
        cls.exclude_from_training_data = ['snapshot_time', 'visit_number', 'prediction_time']
        cls.grid = {
            'max_depth': [3],
            'learning_rate': [0.1],
            'n_estimators': [100]
        }
        cls.ordinal_mappings = {
            'arrival_method': ['walk-in', 'referral', 'ambulance']
        }
    ...
    def test_balanced_training(self):
        ...
        self.assertLessEqual(balance_info['balanced_size'], balance_info['original_size'])
