...
class WeightedPoissonPredictor:
    ...
    def predict(self, prediction_context, x1, y1, x2, y2):
        ...
        return {
            category: pd.DataFrame({"agg_proba": probs}, index=range(10))
        }
...
class TestAggregate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create test weights
        self.weights = pd.Series(np.random.uniform(0.5, 1.5, self.n_samples), index=index)
    ...
    def test_pred_proba_to_agg_predicted_small(self):
        ...
        result_weighted = pred_proba_to_agg_predicted(predictions, weights, normal_approx_threshold=10)
