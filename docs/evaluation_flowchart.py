...
g.attr(rankdir='LR', bgcolor='white', fontname='Helvetica', splines='ortho', ranksep='1.2', nodesep='0.8')
g.attr('node', fontname='Helvetica', fontsize='16', style='filled', shape='box', width='2.8', height='0.9')
...
SKIP    = {'fillcolor': '#f3f4f6', 'color': '#9ca3af', 'fontcolor': '#6b7280', 'style': 'filled,dashed'}
...
with g.subgraph() as s:
    ...
    s.node('M4a', '4a: P(admission in window) aspirational\nAspirational curve', **PATIENT)
...
g.node('M5a', '5a: # beds needed in window — aspirational\nAggregation: 1 × 2 × 4a', **SPEC)
g.node('M5b', '5b: # admissions in window — observed\nAggregation: 1 × 2 × 4b  [skip at UCLH]', **SKIP)
...
g.node('M7b', '7b: # admissions in window — observed\nTime-varying Poisson: 6 × 4b  [skip at UCLH]', **SKIP)

# Other flows
g.node('M8',  '8: # emergency non-ED arrivals\nTime-varying Poisson · historical', **SPEC)
g.node('M9',  '9: # elective arrivals\nTime-varying Poisson · historical', **SPEC)

# Discharge & transfer
g.node('M11', '11: # emergency discharges from specialty\nAggregation of 10 over emergency inpatients', **SPEC)
g.node('M12', '12: # elective discharges from specialty\nAggregation of 10 over elective inpatients', **SPEC)
g.node('M13', '13: Transition matrix\nP(transfer to B | in specialty A)', **SPEC)
g.node('M14', '14: # emergency transfers into specialty\nConvolve 11 thinned by 13', **SPEC)
g.node('M15', '15: # elective transfers into specialty\nConvolve 12 thinned by 13', **SPEC)
...
# Real edges
g.edges([('M1','M3'),('M2','M3'),
         ('M1','M5a'),('M2','M5a'),('M4a','M5a'),
         ('M1','M5b'),('M2','M5b'),('M4b','M5b'),
         ('M6','M7a'),('M4a','M7a'),
         ('M6','M7b'),('M4b','M7b'),
         ('M10','M11'),('M10','M12'),
         ('M11','M14'),('M13','M14'),
         ('M12','M15'),('M13','M15'),
         ('M5a','M16'),('M7a','M16'),('M8','M16'),('M14','M16'),
         ('M9','M17'),('M15','M17'),
         ('M16','M18'),('M11','M18'),
         ('M17','M19'),('M12','M19')])
