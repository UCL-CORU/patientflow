...
class TestSequenceToOutcomePredictor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for _ in range(n_samples):
            ...
            group_len = np.random.randint(input_len, input_len + 3)  # allow up to 2 more
            if group_len == input_len:
                grouping_vars.append(input_seq)
            else:
                extension = tuple(np.random.choice(options, size=group_len - input_len, replace=True))
                grouping_vars.append(input_seq + extension)

        outcome_options = options
        self.test_data = pd.DataFrame({
            'snapshot_date': pd.date_range('2023-01-01', periods=n_samples, freq='h'),
    ...
    def test_ensure_tuple(self):
        ...
        # Test list input
        self.assertEqual(predictor._ensure_tuple(['medical', 'surgical']), ('medical', 'surgical'))
    ...
    def test_probability_calculation(self):
        ...
        test_sequences = [('medical',), ('surgical',), ('paediatric',), ('medical', 'surgical')]
    ...
    def test_input_to_grouping_probabilities(self):
        ...
        if not predictor.input_to_grouping_probs.empty:
            ...
            probs = predictor.input_to_grouping_probs.drop(columns=['probability_of_grouping_sequence'])
