...
class DemandPredictor:
    ...
    def predict_subspecialty(
        self,
        subspecialty_id: str,
        prob_admission_pats_in_ed: np.ndarray,
        lambda_ed_yta: float,
        ...
        1. Current ED patients (using pre-computed probability distribution)
        2. Yet-to-arrive ED patients (Poisson distribution)
        3. Yet-to-arrive non-ED emergency patients (Poisson distribution)
        4. Yet-to-arrive elective patients (Poisson distribution)
        ...
        prob_admission_pats_in_ed : numpy.ndarray
            Pre-computed probability mass function for current ED patients
            who may be admitted to this subspecialty
        lambda_ed_yta : float
            Poisson parameter for expected number of ED patients yet to arrive
        lambda_non_ed_yta : float
            Poisson parameter for expected number of non-ED emergency patients yet to arrive
        lambda_elective_yta : float
            Poisson parameter for expected number of elective patients yet to arrive
        ...
        lambda_combined_yta = lambda_ed_yta + lambda_non_ed_yta + lambda_elective_yta
        ...
        # Convolve ED current with combined Poisson
        p_total = self._convolve(prob_admission_pats_in_ed, p_poisson)
...
class HospitalHierarchy:
    ...
    def get_subspecialties_for_reporting_unit(self, reporting_unit_id: str) -> List[str]:
        ...
        """
        return [sid for sid, rid in self.subspecialties.items() if rid == reporting_unit_id]
...
class HierarchicalPredictor:
    ...
    def predict_all_levels(
        ...
        reporting_units = set(self.hierarchy.subspecialties.values())
        for reporting_unit_id in reporting_units:
            subspecialties = self.hierarchy.get_subspecialties_for_reporting_unit(reporting_unit_id)
            subspecialty_preds = [results[sid] for sid in subspecialties]
            pred = self.predictor.predict_reporting_unit(reporting_unit_id, subspecialty_preds)
            results[reporting_unit_id] = pred
            self.cache[reporting_unit_id] = pred
        
        # Level 3: Divisions
        divisions = set(self.hierarchy.reporting_units.values())
        for division_id in divisions:
            reporting_units = self.hierarchy.get_reporting_units_for_division(division_id)
            reporting_unit_preds = [results[rid] for rid in reporting_units]
            pred = self.predictor.predict_division(division_id, reporting_unit_preds)
            results[division_id] = pred
            self.cache[division_id] = pred
        
        # Level 4: Boards
        boards = set(self.hierarchy.divisions.values())
        for board_id in boards:
            divisions = self.hierarchy.get_divisions_for_board(board_id)
            division_preds = [results[did] for did in divisions]
            pred = self.predictor.predict_board(board_id, division_preds)
            results[board_id] = pred
            self.cache[board_id] = pred
        
        # Level 5: Hospital
        boards = self.hierarchy.get_boards_for_hospital(hospital_id)
        board_preds = [results[bid] for bid in boards]
...
def populate_hierarchy_from_dataframe(df: pd.DataFrame, hospital_id: str = None) -> HospitalHierarchy:
...
def populate_hierarchical_predictor_from_dataframe(
    df: pd.DataFrame,
    hospital_id: str,
    subspecialty_data: Dict[str, Dict],
    epsilon: float = 1e-7
) -> HierarchicalPredictor:
    """Create and populate a complete HierarchicalPredictor from DataFrame.
    
    This convenience function creates a fully configured HierarchicalPredictor
    by combining a hospital hierarchy (derived from the DataFrame) with a
    DemandPredictor configured with the specified epsilon value.
    ...
    df : pandas.DataFrame
        ...
        - 'sub_specialty': Subspecialty identifier
    hospital_id : str
        Hospital identifier to link all boards to the hospital
    subspecialty_data : dict[str, dict]
        Dictionary mapping subspecialty_id to prediction parameters.
        Not used in this function but included for signature compatibility.
    epsilon : float, default=1e-7
        Truncation threshold for probability distributions in the DemandPredictor
        
    Returns
    -------
    HierarchicalPredictor
        Fully configured predictor ready for hierarchical predictions
        
    Notes
    -----
    This function is a convenience wrapper that:
    1. Creates a HospitalHierarchy from the DataFrame
    2. Links all boards to the specified hospital
    3. Creates a DemandPredictor with the specified epsilon
    4. Returns a configured HierarchicalPredictor
    
    The subspecialty_data parameter is included for API compatibility but
    is not used in this function.
    """
    # Create hierarchy from DataFrame
    hierarchy = populate_hierarchy_from_dataframe(df)
    
    # Add all boards to hospital
    unique_boards = df['board'].dropna().unique()
    for board in unique_boards:
        hierarchy.add_board(board, hospital_id)
    
    # Create predictor with specified epsilon
    predictor = DemandPredictor(epsilon=epsilon)
    hierarchical_predictor = HierarchicalPredictor(hierarchy, predictor)
...
def create_hierarchical_predictor(
    ...
    HierarchicalPredictor
        ...
        - Ready to accept subspecialty_data for predictions
    ...
    3. Use predictor.predict_all_levels() to compute all predictions
