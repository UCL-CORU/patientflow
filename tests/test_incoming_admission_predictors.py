...
class TestIncomingAdmissionPredictors(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        end_date = datetime(2024, 1, 31)  # 31 days of data
        ...
        for i in range(n_patients):
            ...
            # Random specialty
            specialty = np.random.choice(['medical', 'surgical', 'haem/onc', 'paediatric'])
    ...
    def test_poisson_binom_generating_function(self):
        ...
        result = poisson_binom_generating_function(NTimes, arrival_rates, theta, epsilon)
        self.assertIsInstance(result, pd.DataFrame)
    ...
    def test_find_nearest_previous_prediction_time(self):
        ...
        self.assertEqual(result, (12, 0))
        self.assertIn("Time of day requested of (14, 30) was not in model training", str(warning_context.warning))
        self.assertIn("Reverting to predictions for (12, 0)", str(warning_context.warning))
        ...
        self.assertIn("Time of day requested of (6, 0) was not in model training", str(warning_context.warning))
        self.assertIn("Reverting to predictions for (20, 0)", str(warning_context.warning))
        ...
        self.assertIn("Time of day requested of (10, 0) was not in model training", str(warning_context.warning))
        self.assertIn("Reverting to predictions for (8, 0)", str(warning_context.warning))
    ...
    def test_incoming_admission_predictor_base_class(self):
        ...
        predictor = ParametricIncomingAdmissionPredictor(filters=self.filters, verbose=True)
        ...
        filtered_df_func = predictor.filter_dataframe(self.test_df, {'specialty': is_medical})
    ...
    def test_empirical_predictor_predict(self):
        ...
        predictor_no_survival = EmpiricalIncomingAdmissionPredictor(filters=self.filters)
        with self.assertRaises(RuntimeError):
    ...
    def test_edge_cases(self):
        ...
        single_time_predictor = ParametricIncomingAdmissionPredictor(filters=self.filters)
    ...
    def test_predictor_warning_functionality(self):
        ...
        warning_messages = [str(warning.message) for warning in warning_context.warnings]
